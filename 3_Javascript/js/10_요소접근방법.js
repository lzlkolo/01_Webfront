...
function classTest() {
    ...
    // 유사배열이란?
    // 배열처럼 index, length를 가자ㅣ고 있으나
    // 배열 전용 기능(메서드)을 제공하지 않음
    // ex) pop(), push() ...

    // .cls-test 요소 모두 얻어오기
    // query selector가 아니므로 클레스 호출할 때 . 안 써도 됨
    // -> HTMLCollection ( == 유사배열 )
    const divs = document.getElementsByClassName("cls-test");

    console.log(divs);

    // divs 0, 1, 2번째 요소에 배경색 변경
    divs[0].style.backgroundColor = "rgb(114,203,80)";
    divs[1].style.backgroundColor = "blue";
    divs[2].style.backgroundColor = "lavender";
}
...
function nameTest() {
    ...
    for (let i = 0; i < hobbyList.length; i++) {
        ...
        // input요소.checked -> 체크되어 있으면 true, 아니면 false
...
function readValue() {
    ...
    // bg.scrollHeight    : bg의 스크롤 전체 높이 반환
 ...
 * 올라온 키가 "Enter" 라면 readValue() 함수를 호출
 */
